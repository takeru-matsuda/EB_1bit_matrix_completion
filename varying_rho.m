addpath(genpath('./comparison-methods'));
...
MSE_MMGN = zeros(length(rhos),nmc);
MSE_Trace = zeros(length(rhos),nmc);
MSE_Max = zeros(length(rhos),nmc);
MSE_LMF = zeros(length(rhos),nmc);
MSE_EB1 = zeros(length(rhos),nmc);
MSE_EB2 = zeros(length(rhos),nmc);
hellinger_MMGN = zeros(length(rhos),nmc);
hellinger_Trace = zeros(length(rhos),nmc);
hellinger_Max = zeros(length(rhos),nmc);
hellinger_LMF = zeros(length(rhos),nmc);
hellinger_EB1 = zeros(length(rhos),nmc);
hellinger_EB2 = zeros(length(rhos),nmc);
hellinger_unobs_MMGN = zeros(length(rhos),nmc);
hellinger_unobs_Trace = zeros(length(rhos),nmc);
hellinger_unobs_Max = zeros(length(rhos),nmc);
hellinger_unobs_LMF = zeros(length(rhos),nmc);
hellinger_unobs_EB1 = zeros(length(rhos),nmc);
hellinger_unobs_EB2 = zeros(length(rhos),nmc);
KL_MMGN = zeros(length(rhos),nmc);
KL_Trace = zeros(length(rhos),nmc);
KL_Max = zeros(length(rhos),nmc);
KL_LMF = zeros(length(rhos),nmc);
KL_EB1 = zeros(length(rhos),nmc);
KL_EB2 = zeros(length(rhos),nmc);
KL_unobs_MMGN = zeros(length(rhos),nmc);
KL_unobs_Trace = zeros(length(rhos),nmc);
KL_unobs_Max = zeros(length(rhos),nmc);
KL_unobs_LMF = zeros(length(rhos),nmc);
KL_unobs_EB1 = zeros(length(rhos),nmc);
KL_unobs_EB2 = zeros(length(rhos),nmc);
time_MMGN = zeros(length(rhos),nmc);
time_Trace = zeros(length(rhos),nmc);
time_Max = zeros(length(rhos),nmc);
time_LMF = zeros(length(rhos),nmc);
time_EB1 = zeros(length(rhos),nmc);
time_EB2 = zeros(length(rhos),nmc);
for ir=1:length(rhos)
    rho = rhos(ir);
    parfor mc=1:nmc
        rng(mc);
        ...
        hellinger_unobs_EB0(ir,mc) = mean(hellinger_bernoulli(normcdf(M(obs==0),0,sigma),pred_EB2(obs==0)));
